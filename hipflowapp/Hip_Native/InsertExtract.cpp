...
{
	return AddIt(dataCnt, ppData, (uint8_t*) &x, 1);
}
...
uint8_t insert( float    x, uint8_t **ppData, uint8_t& dataCnt) // these translate nans
{
	...
	return AddIt(dataCnt, ppData, (uint8_t*) &t, 4);
}
...
{	
    ...
    return AddIt(dataCnt, ppData, (uint8_t*) &t, 8);	
}
...
uint8_t extract( uint16_t& x, uint8_t **ppData, uint8_t& dataCnt)
{
	...
	pD+=1;// add one unsigned short size
	*ppData = (uint8_t*)pD;
	dataCnt-= 2;
	return 0; //SUCCESS
}
