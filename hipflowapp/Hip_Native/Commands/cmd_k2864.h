...
class cmd_2864 : public cmd_base
{
	uint8_t activeDevVar;
public: // c.dtor
	cmd_2864():cmd_base(2864) {	};
	~cmd_2864(){};
...
uint8_t cmd_2864::extractData(uint8_t &ByteCnt, uint8_t *pData)
{
	uint8_t ret = 0;
...
uint8_t cmd_2864::insert_Data(uint8_t &ByteCnt, uint8_t *pData) 
{
	int ret = 0;
	ByteCnt = 0; // just in case
	do // once
	{
		...
		if (ret = insert(activeDevVar, &pData, ByteCnt))  break;// yes they are the same...
		// for us, the flow dv is always zero
		*pData++ = 0; ByteCnt++;
	}
