...
class cmd_2563 : public cmd_base
{
	uint8_t activeDevVar;
public: // c.dtor
	cmd_2563():cmd_base(2563) {	};
	~cmd_2563(){};
...
uint8_t cmd_2563::extractData(uint8_t &ByteCnt, uint8_t *pData)
{
	uint8_t ret = 0;
...
uint8_t cmd_2563::insert_Data(uint8_t &ByteCnt, uint8_t *pData) 
{
	int ret = 0;
	ByteCnt = 0; // just in case
	do // once
	{
		...
		if (ret = devVar_Total.classification.insertSelf(&pData, &ByteCnt))break;
		if (ret = devVar_Total.Units.insertSelf(&pData, &ByteCnt)) break;
		if (ret = totValue.insertSelf(&pData, &ByteCnt)) break;// a double
		if (ret = totStatus.insertSelf(&pData, &ByteCnt)) break;
		if (ret = totTimeStamp.insertSelf(&pData, &ByteCnt)) break;
