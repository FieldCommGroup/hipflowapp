...
class cmd_2562 : public cmd_base
{
	uint8_t activeDevVar;
public: // c.dtor
	cmd_2562():cmd_base(2562) {	};
	~cmd_2562(){};
...
uint8_t cmd_2562::extractData(uint8_t &ByteCnt, uint8_t *pData)
{
	uint8_t ret = 0;
...
uint8_t cmd_2562::insert_Data(uint8_t &ByteCnt, uint8_t *pData) 
{
	int ret = 0;
	ByteCnt = 0; // just in case
	do // once
	{
		...
		if (ret = totRelayForced.insertSelf(&pData, &ByteCnt)) break;
		if (ret = totRelayInverted.insertSelf(&pData, &ByteCnt)) break;
