...
class cmd_2689 : public cmd_base
{
	uint8_t activeDevVar;
public: // c.dtor
	cmd_2689():cmd_base(2689) {	};
	~cmd_2689(){};
...
uint8_t cmd_2689::extractData(uint8_t &ByteCnt, uint8_t *pData)
{
	...
	{
		ret = totMode.extractSelf(&pData, &ByteCnt, chngCnt);
...
uint8_t cmd_2689::insert_Data(uint8_t &ByteCnt, uint8_t *pData) 
{
	int ret = 0;
	ByteCnt = 0; // just in case
	do // once
	{
		...
		ret = totMode.insertSelf(&pData, &ByteCnt);
	}
