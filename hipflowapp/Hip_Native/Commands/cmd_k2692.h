...
class cmd_2692 : public cmd_base
{
	uint8_t activeDevVar;
public: // c.dtor
	cmd_2692():cmd_base(2692) {	};
	~cmd_2692(){};
...
uint8_t cmd_2692::extractData(uint8_t &ByteCnt, uint8_t *pData)
{
	...
	{
		ret = totRelayForced.extractSelf(&pData, &ByteCnt, chngCnt);
...
uint8_t cmd_2692::insert_Data(uint8_t &ByteCnt, uint8_t *pData) 
{
	int ret = 0;
	ByteCnt = 0; // just in case
	do // once
	{
		...
		ret = totRelayForced.insertSelf(&pData, &ByteCnt);
	}
