...
void bcm2835_gpio_fsel(uint8_t pin, uint8_t mode)
{
    ...
    uint8_t   shift = (uint8_t) ((pin % 10) * 3);
...
{
  if (bcm2835_pads == (uint32_t *)MAP_FAILED)
    return 0;
...
int bcm2835_spi_begin(void)
{
    ...
    if (bcm2835_spi0 == (uint32_t *)MAP_FAILED)
...
void bcm2835_spi_transfernb(char* tbuf, char* rbuf, uint32_t len)
{
    ...
    */
    ...
    while((TXCnt < len)||(RXCnt < len))
    {
        ...
        {
           rbuf[RXCnt] = TAKECHAR( bcm2835_peri_read_nb(fifo) );
           RXCnt++;
        }
    }
    /* Wait for DONE to be set */
...
int bcm2835_aux_spi_begin(void) {
    ...
    if (bcm2835_spi1 == (uint32_t *)MAP_FAILED)
      return 0; /* bcm2835_init() failed, or not root */
...
int bcm2835_i2c_begin(void)
{
    ...
    if (   bcm2835_bsc0 == (uint32_t *)MAP_FAILED
...
uint8_t bcm2835_i2c_read(char* buf, uint32_t len)
{
    ...
    while (!(bcm2835_peri_read_nb(status) & BCM2835_BSC_S_DONE))
    {
	    ...
	    buf[i] = TAKECHAR(bcm2835_peri_read_nb(fifo));
	    i++;
	    remaining--;
    	}
    }
...
{   
    ...
    while (!(bcm2835_peri_read(status) & BCM2835_BSC_S_DONE))
    {
    	...
    	{
	    /* Read from FIFO */
	    buf[i] = TAKECHAR( bcm2835_peri_read(fifo) );
	    i++;
	    remaining--;
    	}
    }
...
uint64_t bcm2835_st_read(void)
{
    ...
    if (bcm2835_st==(uint32_t *)MAP_FAILED)
	return 0;
...
{
    if (   bcm2835_clk == (uint32_t *)MAP_FAILED
...
int bcm2835_init(void)
{
	...
	string d(NATIVE_RANGES_FILE), s(BMC2835_RPI2_DT_FILENAME);

	// we aren't going to do this because we cannot join the kmem group   FILE* f = openSystemFile( d, s );
	// maybe later...(it shows I'm in the kmem group without listing it in groups)
    ...
    */
	...
	{
		bcmLOGIT( " file %s failed to open.\n",BMC2835_RPI2_DT_FILENAME);
		goto exit;
	}
    ...
    memfd = -1;
    ok = 0;
   // try to do it anyway... if (geteuid() == 0)
   if ( 1 )
    {
		...
		{
			bcmLOGIT( " UID is zero and /dev/mem opened OK.\n");
		}
      
		/* Base of the peripherals block is mapped to VM */
		bcm2835_peripherals = (uint32_t *)mapmem("gpio", bcm2835_peripherals_size, memfd, (off_t)bcm2835_peripherals_base);
		if (bcm2835_peripherals == (uint32_t *)MAP_FAILED) 
		{
			bcmLOGIT( " mapmem FAILED, exitting init function.\n");
			goto exit;
		}
		else
		{
			bcmLOGIT( " UID is zero and mapmem executed correctly. Setting addresses.\n");
		}
    ...
    else
    {
		...
		{
			bcmLOGIT( " UID is NOT zero and /dev/gpiomem opened OK.\n");
		}
		...
		if (bcm2835_peripherals == (uint32_t *)MAP_FAILED) 
		{
			bcmLOGIT( " mapmem FAILED for gpiomem, exitting init function.\n");
			goto exit;
		}
		else
		{
			bcmLOGIT( " UID is NOT zero and mapmem for gpiomem executed correctly.\n");
		}
